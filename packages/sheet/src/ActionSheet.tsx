...
export default forwardRef<ActionSheetRef, ActionSheetProps>(
  function ActionSheet(
    ...
    const hideAnimation = React.useCallback(
      (vy?: number, callback?: (_param: { finished: boolean }) => void) => {
        if (!animated) {
    ...
    const getCurrentPosition = React.useCallback(() => {
      return (animations.translateY as any)._value <=
        minTranslateValue.current + 5
        ? 0
        : ((animations.translateY as any)._value as number);
      // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []);
          ...
          if (drawUnderStatusBar) {
            ...
            if (actionSheetHeight.current >= correctedHeight - 1) {
              if (value.value < 100) {
                ...
                if ((animations.underlayTranslateY as any)._value !== 100) {
